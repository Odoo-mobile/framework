...
public class ORM extends SQLiteDatabaseHelper {
	...
	for (int i = 0; i < values.length(); i++) {
		...
		/* Specially for Messages */
		/*
		 * if (m2m.getModel_name().equals("res.partner") &&
		 * values.length() == 1) {
		 * 
		 * if (OpenERPAccountManager.currentUser(context)
		 * .getPartner_id().equals(String.valueOf(row_id))) { if
		 * (table1.equals("mail_message") &&
		 * !rootRow.getAsString("author_id").equals( "false")) { try {
		 * String author_id = new JSONArray(
		 * rootRow.getAsString("author_id")) .getString(0); if
		 * (author_id.equals(String.valueOf(row_id))) {
		 * Log.e("VAlue for res_partner_id : ", row_id + ""); continue;
		 * } } catch (Exception e) { }
		 * 
		 * }
		 * 
    ...
    // private String selectStatement(String table, String[] columns) {
    ...
    // selectStatement.append(string);
    // selectStatement.append(", ");
    // }
    // selectStatement.deleteCharAt(selectStatement.lastIndexOf(","));
    // }
	...
	Log.e("ORM::executeSQL", sqlQuery.toString() + " , ARGUMENTS:"
		+ TextUtils.join(", ", args));
	...
	Log.e(">>>>>>>>> Execute Query ", "where : " + where);
	String columns[] = cols.toArray(new String[cols.size()]);
